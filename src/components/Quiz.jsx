...
import { KATAKANA_DATA } from "../data.js";
...
export default function Quiz({ mode, wrongCounts, onRegisterMiss }) {
  const [selectedLesson, setSelectedLesson] = useState("1"); // Default to Lektion 1 for learning
  ...
  const generateQuestion = (lessonFilter) => {
    ...
    // Weighted random selection based on past mistakes
    const nextChar = pickWeighted(
      filteredPool,
      wrongCounts,
      (item) => item.char,
    );
    ...
    const correctVal = mode === "char2rom" ? nextChar.rom : nextChar.char;

    // Get unique list of all options in the filtered pool
    const allValsInPool = Array.from(
      new Set(
        filteredPool.map((item) =>
          mode === "char2rom" ? item.rom : item.char,
        ),
      ),
    );
  ...
  const handleOptionClick = (option) => {
    ...
    const correctVal =
      mode === "char2rom" ? currentQuestion.rom : currentQuestion.char;
    const isCorrect = option === correctVal;

    if (isCorrect) {
      setCorrectCount((prev) => prev + 1);
    } else {
      // Register mistake for weight adjustment
      onRegisterMiss(currentQuestion.char);
    }
    ...
    <div className="quiz-container">
      ...
      {!currentQuestion ? (
        <div className="no-data-msg">
          Keine Zeichen für diese Lektion verfügbar.
        </div>
      ) : (
        <>
          <div className="quiz-question-box">
            <div className="quiz-prompt">
              {mode === "char2rom"
                ...
                : "Finde das Katakana-Zeichen für:"}
            </div>
            <div
              className="quiz-char"
              style={
                mode === "rom2char"
                  ? { fontSize: "3rem", fontFamily: "var(--font-serif)" }
                  : {}
              }
            >
              {mode === "char2rom" ? currentQuestion.char : currentQuestion.rom}
            </div>
          </div>
